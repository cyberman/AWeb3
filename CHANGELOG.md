...
- **Socket timeouts:** Socket connections will now timeout and close gracefully if no response
...
- **Default Fonts:** Changed default fonts from bitmap fonts (times.font, courier.font) to scalable fonts (CGTimes.font, LetterGothic.font) for better rendering quality
...
- **AWeb: Assign:** Set default assign path to simply "AWeb:" and now creates the assign automatically on launch if it does not exist, pointing  issue where AWeb would remove an existing "AWeb:" assign on exit. Now checks if assign already exists before creating it, and only removes assigns that it created itself
- **Font Defaults:** Default fonts now use the system supplied scalable fonts, with improved font fallback mechanism to gracefully fall back to bitmap fonts (times.font, courier.font) if scalable fonts are not available, with final fallback to topaz.font
